...
const socialInsights = [
  { label: "Audience engagement rising", detail: "+11% vs last week", color: "from-emerald-500/15 to-emerald-500/5" },
  { label: "Cost per engagement is optimal", detail: "$0.75 CPE (Target: <$1.00)", color: "from-amber-500/15 to-amber-500/5" },
  { label: "Profile traffic consistent", detail: "2,500 profile visits (Target: >2,000)", color: "from-sky-500/15 to-sky-500/5" },
  { label: "Follower growth accelerating", detail: "450 new followers this month", color: "from-purple-500/15 to-purple-500/5" },
]
      ...
      <CardHeader className="space-y-4">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <CardDescription>Multi-platform campaigns snapshot.</CardDescription>
          </div>
      ...
      <CardContent className="space-y-8">
        <section className="grid gap-6 lg:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)]">
          <div className="space-y-4 rounded-2xl border border-slate-100 p-4">
            <div>
              <p className="text-sm font-semibold text-slate-900">Engagement Metrics</p>
              <p className="text-xs text-slate-500">Weekly totals across all platforms.</p>
            </div>
            <ChartContainer config={engagementConfig} className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={engagementData} margin={{ left: 0, right: 16, top: 16, bottom: 8 }}>
                  ...
                  <Bar dataKey="engagement" fill="var(--color-engagement)" radius={[6, 6, 0, 0]} />
                  <Bar dataKey="impressions" fill="var(--color-impressions)" radius={[6, 6, 0, 0]} />
                  <Bar dataKey="reach" fill="var(--color-reach)" radius={[6, 6, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </ChartContainer>
          ...
          <div className="space-y-4 rounded-2xl border border-slate-100 p-4">
            <div>
              <p className="text-sm font-semibold text-slate-900">Follower Growth Trend</p>
              <p className="text-xs text-slate-500">Last 30 days growth trend.</p>
            </div>
            <ChartContainer
              config={{ growth: { label: "Growth", color: "hsl(var(--chart-3))" } }}
              className="h-64"
            >
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={followerGrowth} margin={{ left: 0, right: 16, top: 16, bottom: 8 }}>
                  <defs>
