    ...
    <Card className="h-full border-slate-200/70">
      <CardHeader className="flex flex-row items-center justify-between gap-2">
        <div>
          ...
          <CardDescription>
            Feedback that still needs your response.
          </CardDescription>
        </div>
        <Button className="gap-2 text-slate-600" size="sm" variant="ghost">
          View all
          ...
          >
            <div className="flex items-start justify-between gap-2">
              <div>
                ...
                <p className="text-slate-500 text-xs">{request.role}</p>
              </div>
              <Badge
                className={`border ${urgencyStyles[request.urgency]}`}
                variant="secondary"
              >
